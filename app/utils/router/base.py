...
class Intent:
    ...
    The cog is the arbiter of whether it wants a message: ``cheap_predicate``
    (sync, metadata only, no I/O) is the cog's own gate, run before any download
    or model call. The cog puts whatever logic it likes there (channel, regex,
    user, content type, ...), the same way a URL-handler cog checks a message in
    its own listener.
